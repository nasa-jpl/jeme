// CMS-Flux Dashboard - matches the comprehensive main dashboard layout
...
const CMSFluxDashboard = () => {
  ...
  // Load CMS-Flux data for time series chart
  useEffect(() => {
  ...
  return (
    <div className="bg-gray-100 min-h-screen">
      ...
      <main className="max-w-7xl mx-auto px-4 py-6">
            ...
            {models.map((model, index) => (
              <Link 
                ...
                className={`group p-4 border rounded-lg hover:border-blue-300 hover:shadow-md transition-all duration-200 ${
                  model.name === 'CMS-Flux' ? 'border-blue-300 bg-blue-50' : 'border-gray-200'
                }`}
              >
                <div className="flex items-start gap-3">
                  <div className={`p-2 rounded-lg transition-colors ${
                    model.name === 'CMS-Flux' ? 'bg-blue-100' : 'bg-gray-50 group-hover:bg-blue-50'
                  }`}>
                    {model.icon}
                  </div>
                  <div className="flex-1 min-w-0">
                    <h3 className={`font-semibold transition-colors ${
                      model.name === 'CMS-Flux' ? 'text-blue-900' : 'text-gray-900 group-hover:text-blue-900'
                    }`}>
                      {model.name}
                    </h3>
        ...
        <ModelInfoSection modelName="CMS-Flux" modelDisplayName="CMS-Flux" />
        <PaperInfo modelName="CMS-Flux" />
        <Header modelName="CMS-Flux" />
        ...
        <div className="bg-white rounded-lg p-5 shadow-sm mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <Link
              to="/science-model-dashboard/CMS-Flux/citations"
            ...
            <Link
              to="/science-model-dashboard/CMS-Flux/geographic-impact"
            ...
            <Link
              to="/science-model-dashboard/CMS-Flux/research-domains"
            ...
            <Link
              to="/science-model-dashboard/CMS-Flux/uncertainty"
        ...
        <div className="grid grid-cols-2 gap-6 mb-6">
          <ResearchDomainsCard data={cmsFluxData} />
          <EngagementLevelsCard data={cmsFluxData} />
        </div>

        {cmsFluxData.length > 0 && cmsFluxData[0]?.uncertainty && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
            <UncertaintyOverviewCard data={cmsFluxData} />
            <UncertaintyMatrixCard data={cmsFluxData} />
          </div>
        )}

        <FutureTrendsChart data={cmsFluxData} />

        <div className="mb-6">
          <MissionsSummary citationsData={cmsFluxData} maxMissions={8} showDetails={true} />
        </div>

        <DashboardSummaryCard data={cmsFluxData} />
        
        <div className="grid grid-cols-2 gap-6 mb-6">
          <JournalDistributionCard data={cmsFluxData} />
          <GitHubMetricsCard owner="ornldaac" repo="cms" />
        </div>
