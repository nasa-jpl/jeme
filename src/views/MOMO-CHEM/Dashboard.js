...
const MOMOCHEMDashboard = () => {
  ...
  // Load MOMO-CHEM data for time series chart
  useEffect(() => {
      ...
      <main className="max-w-7xl mx-auto px-4 py-6">
            ...
            {models.map((model, index) => (
              <Link 
                ...
                className={`group p-4 border rounded-lg hover:border-blue-300 hover:shadow-md transition-all duration-200 ${
                  model.name === 'MOMO-CHEM' ? 'border-blue-300 bg-blue-50' : 'border-gray-200'
                }`}
              >
                <div className="flex items-start gap-3">
                  <div className={`p-2 rounded-lg transition-colors ${
                    model.name === 'MOMO-CHEM' ? 'bg-blue-100' : 'bg-gray-50 group-hover:bg-blue-50'
                  }`}>
                    {model.icon}
                  </div>
                  <div className="flex-1 min-w-0">
                    <h3 className={`font-semibold transition-colors ${
                      model.name === 'MOMO-CHEM' ? 'text-blue-900' : 'text-gray-900 group-hover:text-blue-900'
                    }`}>
                      {model.name}
                    </h3>
        ...
        <div className="bg-white rounded-lg p-5 shadow-sm mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <Link
              to="/MOMO-CHEM/geographic-impact"
