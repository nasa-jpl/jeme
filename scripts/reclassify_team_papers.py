...
from llm_reclassify_engagement import (  # noqa: E402
    build_prompt, call_gemini, MODEL_CONTEXT, VALID_LABELS,
)
...
def classify_one(api_key, model, entry, valid):
...
def run_model(model, api_key, dry_run=False):
    ...
    with ThreadPoolExecutor(max_workers=WORKERS) as exe:
        futs = {exe.submit(classify_one, api_key, model, p, valid): p for p in team}
...
def main():
    ...
    summary = []
    for m in models:
        s = run_model(m, api_key, dry_run=args.dry_run)
